...
- **`forge_server/job_store.py`**: In-memory job store with `forge_jobs.json` persistence. Each job serialises `loss_history` and `log_buffer` (last 500 entries each) so logs and charts survive a server restart.
...
- **`forge_server/routes/`**: REST API + WebSocket `/ws`
  ...
  - `files.py` — `/api/models`, `/api/loras`, `/api/datasets` (filesystem scanning). Dataset scanner detects sd-scripts roots recursively: a directory is a dataset root if it contains `{N}_{name}` repeat-named subdirectories. `/api/datasets` results are cached for 30 s and the scan runs in a thread-pool executor to avoid blocking the event loop. `/api/browse-folder` opens a native OS folder-picker dialog (via a tkinter subprocess) and returns `{"path": "..."}` — used by the train form's "Custom path…" dataset option.
  - `system.py` — `/api/system/stats` (GPU via nvidia-smi, disk usage)
  - `settings_route.py` — `/api/settings` GET/POST (reads/writes `forge_config.json`). The POST body is validated against a `SettingsUpdate` Pydantic model — unknown fields are rejected.
...
- **`getByLabel` / `setByLabel` label uniqueness**: these helpers use a case-insensitive `includes` search across all `.kv` spans in the form. Label text must be globally unique within the form — avoid names that are substrings of other labels. For example, the sampling-cadence inputs use `sample_every_n_steps` / `sample_every_n_epochs` (with the `sample_` prefix) so they are not ambiguously matched by the saving-cadence labels `save_every_n_steps` / `save_every_n_epochs`. Follow this pattern when adding new fields.
- **Resolution multi-select**: `#resolution-btns` carries `data-multiselect="true"`, which causes the general `.row-flex` single-select handler to skip it. A dedicated handler in `mountTrain()` instead toggles individual buttons (at least one must remain active). Clicking "Custom…" calls `_addCustomResRow()`, which appends a new `.custom-res-row` div (W input, × separator, H input, ✕ remove button) to `#custom-res-list`; any number of custom rows can coexist with active preset buttons. `_updateCustomResBtn()` keeps the "Custom…" button highlighted iff at least one row exists. `collectFormState()` collects all active preset buttons plus every valid `.custom-res-row` into `resolutions: string[]` (schema field). `_applyConfigToForm()` restores the multi-selection from `cfg.resolutions` — preset-sized square values reactivate their button, all others are restored as individual custom rows; it also accepts the old `cfg.resolution` string for backward compatibility with stored jobs.
...
- **Dataset source**: the `#ds-select` dropdown is populated from `/api/datasets`; a "Custom path…" sentinel option (`value="__custom__"`) is always appended at the bottom. Selecting it reveals `#ds-custom-row` (a text input `#ds-custom-path` + "Browse…" button). The Browse button calls `GET /api/browse-folder` and fills the input. `collectFormState()` returns `ds-custom-path`'s value when `__custom__` is selected. `_applyConfigToForm()` detects paths not present in the dropdown and automatically switches to custom mode.
- `_pendingEdit` (module-level) passes a config dict to `mountTrain()` for pre-population. Set before calling `router.navigate('train')` — never call `mountTrain()` directly, as that skips the DOM rebuild and duplicates event listeners. Sources that set it: the Jobs page "Edit" button (full job config), the preset popover load action, and the Models page "Use for training" button (sparse `{ checkpoint }` dict — `_applyConfigToForm` guards all other fields with undefined checks so a partial object is safe).
- Utility scripts are invoked via `POST /api/utilities/run` with `{tool, args}` — no job queue; result returned inline.
- **User presets** are stored in `localStorage` under key `forge_presets` (object keyed by preset name). The preset popover calls `collectFormState()` on save; on load it sets `_pendingEdit` and calls `router.navigate('train')` (never `mountTrain()` directly — the router must rebuild the DOM first). Two fields require side-effects when restored: (1) The VAE override checkbox requires `dispatchEvent(new Event('change'))` after setting `.checked` so the `mountTrain` listener can update the input's `disabled` state. (2) The architecture pill must be restored via `.click()` (not a manual class toggle) so that the click handler's `updateArchExtras()` call fires — without this, DiT-specific rows like the weighting scheme panel stay hidden even though the correct arch is selected.
